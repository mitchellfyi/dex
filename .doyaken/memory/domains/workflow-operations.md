...
Durable lessons about the Doyaken lifecycle: phase ownership, in-place vs
worktree branch modes, and shared global state Doyaken touches outside the repo.

## M-002: Each lifecycle phase owns its outputs strictly — no spillover

Domain: workflow-operations
Status: active
Scope: dk.sh phase routing, skills/dk*/SKILL.md, prompts/phase-audits/*.md, hooks/phase-loop.sh, hooks/user-prompt-submit.sh
Applies to phases: plan, implement, review, verify, pr, complete
Applies to paths: dk.sh, skills/, prompts/phase-audits/, hooks/phase-loop.sh, hooks/user-prompt-submit.sh
Last verified: 2026-05-15
Recheck when: a new phase is introduced, phase ownership changes, or phase audit prompts are rewritten

Lesson:
Doyaken's six-phase lifecycle (Plan → Implement → Review → Verify & Commit →
...
Evidence:
- `c8f3660 fix(doyaken): defer draft PR creation to Phase 5 (/dkpr)` — Phase 5
...
- `e33d9dc fix(doyaken): skip go-ahead prompt between plan approval and
  implementation` — handoff stays inside the lifecycle.
- `1b2c00e fix: make dkreview dispatch to review loop` — `/dkreview` must
  dispatch into the review wave loop, not freelance.
- `.doyaken/review-rules.md` § `skills/*/SKILL.md` and § `prompts/phase-audits/`
  already codify these constraints.

Future agent behavior:
- When editing a lifecycle skill or phase-audit prompt, re-verify the phase
  boundaries in `dk.sh` and `hooks/phase-loop.sh` before changing what the
...
- When a new phase audit prompt is added, confirm its completion criteria match
  the state/result files read by `dk.sh` and `hooks/phase-loop.sh`.

## M-003: In-place lifecycle mode is a first-class peer to worktree mode

Domain: workflow-operations
Status: active
Scope: dk.sh lifecycle and cleanup helpers, bin/uninit.sh, hooks/session-end.sh, lib/session.sh, lib/worktree.sh
Applies to phases: implement, verify, complete (cleanup paths), and any session lifecycle change
Applies to paths: dk.sh, bin/uninit.sh, hooks/session-end.sh, lib/session.sh, lib/worktree.sh
Last verified: 2026-05-15
Recheck when: branch-mode handling changes, worktree creation/cleanup logic changes, or session ID derivation changes

Lesson:
Doyaken supports two workspace modes — worktree mode and in-place mode (where
...
- `02028e5 fix(cleanup): preserve active in-place lifecycle state` (149 LoC in
  dk.sh) protects active in-place branches during cleanup, handles renamed
...
- `dk.sh` line 218+ and 387+ branches explicitly on `workspace_mode == in-place`.
...
- Do not assume the session is tied to a `.doyaken/worktrees/` directory —
...
- New session/branch state files must be cleaned up by `dk_cleanup_session`
  and legacy migration when appropriate (per `.doyaken/review-rules.md` §
  `lib/*.sh`).

## M-004: Doyaken-owned global state must be tracked separately from user state
...
Applies to paths: bin/install-settings.sh, bin/uninstall.sh, bin/config.sh, ~/.claude/.doyaken-install-state.json
Last verified: 2026-05-15
...
shared user-owned state. Doyaken install and uninstall must preserve existing
user entries (worktree symlinks, MCP server definitions, hooks, permissions) and
must only remove entries that Doyaken itself wrote. Provenance is tracked via
`~/.claude/.doyaken-install-state.json`. Existing-settings installs require
...
- `d442a7b fix(settings): preserve user-owned global config` (188 lines added
  ...
  preserves worktree symlinks during install/uninstall, tracks Doyaken-owned
...
- `bin/install-settings.sh` declares `INSTALL_STATE_FILE="$CLAUDE_DIR/.doyaken-install-state.json"`.
...
- When modifying install or uninstall logic for `~/.claude/settings.json` or
  ...
  the keys Doyaken owns according to the install-state file.
- Track every newly written entry in `.doyaken-install-state.json` so a future
  uninstall can remove only what Doyaken added.
