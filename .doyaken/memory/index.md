...
Promotion of new memory entries goes through `/dksync` or `dk sync`. Raw
...
|--------|------|-----------|--------|
| review-quality | domains/review-quality.md | Phase 3 review waves; editing `agents/review-*.md`, `agents/self-reviewer.md`, `agents/review-verifier.md`, `prompts/review-wave.md`, `prompts/review.md`, `prompts/phase-audits/3-review*.md`, or `skills/dkreview*/` | active |
| workflow-operations | domains/workflow-operations.md | Lifecycle phase ownership, in-place vs worktree mode, and shared global config; editing `dk.sh` phase routing, `skills/dk*/SKILL.md`, `prompts/phase-audits/`, `hooks/phase-loop.sh`, `bin/install-settings.sh`, `bin/uninstall.sh`, `bin/uninit.sh`, `bin/config.sh`, or session/branch state code | active |
| security-guards | domains/security-guards.md | Editing `hooks/guard-handler.py`, `hooks/guards/*.md`, or `.doyaken/guards/*.md`; adding any new dangerous-command detector | active |
...
| M-004 | workflow-operations | Doyaken-owned global state must be tracked separately from user state |
| M-005 | security-guards | Dangerous-command guards must use syntax-aware detection, not pattern matching |
| M-006 | architecture-decisions | Doyaken skills and prompts must be codebase-agnostic and discover tooling at runtime |
...
- A new lesson should not be added here directly — run `/dksync` or `dk sync`
