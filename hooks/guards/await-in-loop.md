...
Keep the sequential `await` when each iteration depends on the previous one, or when you are deliberately rate-limiting. Comment what the code cannot show about the sequencing. Async iteration forms and awaits inside nested closures or methods are not flagged. `do { … } while (…)` counts as a loop; Swift's `do { … } catch` and Ruby's `each do |x|` do not, since neither is followed by `while`.
