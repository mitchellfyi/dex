...
Path-specific review focus for Dex review waves.

## `dx.sh`

- Verify zsh-only syntax intentionally stays in `dx.sh`.
...
- After changes, run `zsh -n dx.sh`.

## `lib/*.sh`

- Must remain bash/zsh-compatible; no zsh-only parameter expansion or arrays.
- Shared state-file helpers should be cleaned up by `dx_cleanup_session` and
...
- Phase 3 single-wave audits may complete with `FINDINGS_FIXED:N`; the outer
  `/dxreviewloop` owns the adaptive consecutive `CLEAN` gate.
