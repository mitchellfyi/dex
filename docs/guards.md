...
| `action` | no | warn, block | Warn returns hook context; block prevents the action. Defaults to `warn` when omitted, so a blocking guard must say `action: block` explicitly. Case is ignored, so `Block` blocks; any other value warns and says on stderr that it was not understood, rather than silently becoming a warn guard |
