...
class QueueTests(IntakeCase):
    ...
    def test_queue_selects_oldest_request_and_claims_only_one(self):
        ...
        self.assertEqual([8], [item["issue_number"] for item in result["queue"]])
        expanded = self.invoke(event="schedule", limit=2)
        self.assertEqual([8, 7], [item["issue_number"] for item in expanded["queue"]])
