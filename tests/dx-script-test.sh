...
assert_contains 'prompts/workflows/dxpr.md' "$ROOT/skills/dxpr/SKILL.md"
assert_contains 'gh pr ready "$PR_NUM"' "$ROOT/prompts/workflows/dxpr.md"
assert_contains "Phase 5 complete. PR state: READY" "$ROOT/prompts/workflows/dxpr.md"
assert_contains 'must return `false`' "$ROOT/prompts/phase-audits/5-pr.md"
assert_contains 'dx_github_pr_attachments_supported' "$ROOT/prompts/workflows/dxpr.md"
assert_contains 'attachment_fingerprint' "$ROOT/prompts/workflows/dxpr.md"
assert_contains '![<bundle alt>](<absolute local image path>)' "$ROOT/prompts/workflows/dxpr.md"
assert_contains '![](<absolute local video path>)' "$ROOT/prompts/workflows/dxpr.md"
assert_contains 'gh pr edit "$PR_NUM" --body-file "$PR_BODY_FILE" --attach' "$ROOT/prompts/workflows/dxpr.md"
assert_contains 'gh pr view "$PR_NUM" --json body -q .body > "$PR_BODY_FILE"' "$ROOT/prompts/workflows/dxpr.md"
assert_contains '50 files' "$ROOT/prompts/workflows/dxpr.md"
...
assert_contains "BRANCH_SPECIFIC_COUNT" "$ROOT/prompts/workflows/dxpr.md"
assert_contains "Do not push the branch" "$ROOT/prompts/workflows/dxpr.md"
