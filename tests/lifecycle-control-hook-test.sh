...
[[ "$(dx_lifecycle_control_read "$DEX_SESSION_ID" action)" == "cancel" ]]
[[ "$(dx_lifecycle_control_read "$DEX_SESSION_ID" source)" == "user-prompt" ]]
[[ -f "$PAUSED_FILE" ]]
[[ "$(cat "$OWNER_FILE")" == "$CONTROL_OWNER" ]]
...
[[ "$(dx_lifecycle_control_read "$DEX_SESSION_ID" target_phase)" == "5" ]]
[[ ! -e "$PAUSED_FILE" ]]
...
[[ "$(dx_lifecycle_control_read "$DEX_SESSION_ID" action)" == "cancel" ]]
[[ "$(cat "$(dx_owner_file "$DEX_SESSION_ID")")" == "$CONTROL_OWNER" ]]
