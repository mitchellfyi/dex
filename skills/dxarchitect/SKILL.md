---
name: "dxarchitect"
description: "Bootstrap or refresh `.dex/architecture.md` as a C4 model (System Context, Container, Component — levels 1-3). Reads the codebase, emits mermaid C4 diagrams plus supporting prose. Does not commit."
---

# Skill: dxarchitect

Bootstrap or refresh the project's canonical architecture map at `.dex/architecture.md`. The output is a **C4 model** at the first three levels:
...
- Invoked by `dxrefine` (via the Skill tool) when `.dex/architecture.md` does not exist.
- Invoked directly inside a Claude session via `/dxarchitect [<focus area>]` to bootstrap a missing map or to refresh a stale one.
...
A single file at `<repo-root>/.dex/architecture.md` with this exact top:

```
---
generated-by: dxarchitect
...
Followed by the six sections defined in **Step 4** below. The Level 1-3 sections (1-3) are canonical C4; sections 4-6 (Multi-tenancy, Plug-points, Data Flow) are **dex-specific supplementary** sections — they exist because every refinement needs them, not because they are part of C4.
...
- If the parent session is in plan mode (the agent was told to `EnterPlanMode` or sees plan-mode indicators), **stop**. Tell the user to call `ExitPlanMode` first (or run `dxarchitect` outside the plan-mode context) and re-invoke.
...
2. Read top-level documentation:
   ...
   - `.dex/dex.md` if present (integrations + rule references)
...
7. **Scope decision.** Decide what counts as **the software system in scope** for this map:
   ...
   - If the repo houses microservices owned by different teams → each service is its own software system; this map covers **one** of them. Ask the user which one, or infer from dex.md / CODEOWNERS if unambiguous.
   - If you cannot determine the scope from the codebase alone, **ask the user** before proceeding.

If a focus-area argument was passed (e.g. `/dxarchitect billing`), bias the **Level 3 Component** detail toward that area — still cover all containers at Levels 1-2.
...
- Discover from auth/permission code, UI routes, dex.md, README, CODEOWNERS.
...
#### Section 5 — Reusable Plug-points (dex supplement)

Not part of canonical C4. Required for the dxrefine "reuse beats invent" check.
...
#### Section 6 — Data Flow (dex supplement)
...
1. Compute `.dex/architecture.md` path relative to the repo root.
...
4. Print a final summary:
   ...
   - The exact git command for the user to commit: `git add .dex/architecture.md && git commit -m "docs: bootstrap architecture map"` (or `"refresh"` for overwrites).
   - **The skill never commits.** The user owns the commit.

## Notes

- `dxarchitect` is the canonical source of `.dex/architecture.md`. Anything else that needs the map (`dxrefine` today, future skills tomorrow) **reads** the file rather than rebuilding inline.
...
- When invoked from `dxrefine` via the Skill tool, the parent session is expected to be **outside** plan mode (the `dxrefine` shell wrapper handles this ordering). If you find yourself in plan mode anyway, refuse per Step 1.
