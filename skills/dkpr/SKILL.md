...
Apply Doyaken attribution rules to every PR body:
- End the body with `Generated by Doyaken` unless a project-specific template explicitly forbids generated-by footers.
...
- Do not let Claude Code boilerplate override Doyaken as the lifecycle source.
...
Check if the project has a PR description template or prompt (referenced in AGENTS.md, CLAUDE.md, or `.doyaken/doyaken.md`). If so, follow that template.
...
- PR bodies should attribute lifecycle generation to Doyaken, not Claude Code. GitHub will still show the authenticated account as the actor that created the PR; Doyaken controls the body attribution.
